                         ...
                         const uint8_t *buf, int buf_size)
{
    ...
    if((avctx->codec->capabilities & CODEC_CAP_DELAY) || buf_size){
        ...
        if(*frame_size_ptr < QAVCODEC_MAX_AUDIO_FRAME_SIZE){
            av_log(avctx, AV_LOG_ERROR, "buffer smaller than QAVCODEC_MAX_AUDIO_FRAME_SIZE\n");
            return -1;
        }
...
int avcodec_decode_audio(AVCodecContext *avctx, int16_t *samples,
                         int *frame_size_ptr,
                         const uint8_t *buf, int buf_size){
    *frame_size_ptr= QAVCODEC_MAX_AUDIO_FRAME_SIZE;
